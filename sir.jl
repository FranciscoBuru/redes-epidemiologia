...
mutable struct NodoSIR
    ...
    function NodoSIR(s::Int64, i::Float64, rec::Float64, mu::Float64, beta::Float64,r::Float64, eta::Float64)
        return new(s,i,rec,mu,beta,r,eta,0,0,0)
    end

    function NodoSIR(s::Int64, i::Int64, rec::Float64, mu::Float64, beta::Float64,r::Float64, eta::Float64)
        return new(s,i,rec,mu,beta,r,eta,0,0,0)
    end
end
...
function etapa(mod::SIR)
    ...
    Threads.@threads for j in 1:N
        mod.nodos[j].saux = mod.nodos[j].eta*mod.nodos[j].s
        #eta*sus

        mod.nodos[j].iaux = ((1-mod.nodos[j].mu)*mod.nodos[j].i) + ((1-mod.nodos[j].eta)*mod.nodos[j].s)
        #(1-mu)*inf + (1-eta)*sus

        #mod.nodos[j].raux = mod.nodos[j].rec + mod.nodos[j].mu*mod.nodos[j].i
        #rec + mu*inf

        mod.nodos[j].raux = mod.nodos[j].mu*mod.nodos[j].i
        #mu*inf
    end
    Threads.@threads for j in 1:N
